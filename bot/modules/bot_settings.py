...
async def _start_web_server(port):
    """Start web server with gunicorn (Linux) or uvicorn (Windows)."""
...
async def _kill_web_server():
    """Kill web server process cross-platform."""
    if sys.platform == "win32":
        await kill_process("uvicorn")
    else:
        await kill_process("gunicorn")
...
async def update_private_file(_, message, pre_message, key, new_file=False):
    ...
    if not message.media and (file_name := message.text):
        ...
        if file_name == "accounts.zip":
            ...
            ).wait() if sys.platform != "win32" else None
        elif file_name in [".netrc", "netrc"]:
            if file_name == "netrc":
