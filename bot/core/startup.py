...
async def load_configurations():
    ...
    if sys.platform == "win32":
        # On Windows, start aria2c directly (SABnzbd must be started manually or as a service)
        import shutil
        if shutil.which(BinConfig.ARIA2_NAME):
            await create_subprocess_exec(
                BinConfig.ARIA2_NAME,
                "--allow-overwrite=true", "--auto-file-renaming=true",
                "--bt-enable-lpd=true", "--bt-detach-seed-only=true",
                "--bt-remove-unselected-file=true", "--bt-max-peers=0",
                "--enable-rpc=true", "--rpc-max-request-size=1024M",
                "--max-connection-per-server=10", "--max-concurrent-downloads=1000",
                "--split=10", "--seed-ratio=0", "--check-integrity=true",
                "--continue=true", "--daemon=true", "--disk-cache=40M",
                "--force-save=true", "--min-split-size=10M", "--follow-torrent=mem",
                "--check-certificate=false", "--optimize-concurrent-downloads=true",
                "--http-accept-gzip=true", "--max-file-not-found=0", "--max-tries=20",
                "--peer-id-prefix=-qB4520-", "--reuse-uri=true",
                "--content-disposition-default-utf8=true",
                "--user-agent=Wget/1.12", "--peer-agent=qBittorrent/4.5.2",
    ...
    if PORT:
        if sys.platform == "win32":
            await create_subprocess_exec(
                sys.executable, "-m", "uvicorn", "web.wserver:app",
    ...
    if await aiopath.exists("cfg.zip"):
        jd_cfg = ospath.join("JDownloader", "cfg")
        if await aiopath.exists(jd_cfg):
            await rmtree(jd_cfg, ignore_errors=True)
        await (
            await create_subprocess_exec("7z", "x", "cfg.zip", "-oJDownloader")
        ).wait()
    ...
    if Config.DISABLE_TORRENTS:
        ...
        LOGGER.warning("qBittorrent not available. Skipping configuration.")
