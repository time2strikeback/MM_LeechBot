...
async def confirm_restart(_, query):
    ...
    if data[1] == "confirm":
        ...
        if platform == "win32":
            kill_cmd = f"taskkill /F /IM aria2c.exe /IM qbittorrent.exe /IM ffmpeg.exe /IM rclone.exe /IM java.exe /IM 7z.exe 2>NUL"
            proc1 = await create_subprocess_shell(kill_cmd)
        else:
        ...
        proc2 = await create_subprocess_exec(executable, "update.py")
