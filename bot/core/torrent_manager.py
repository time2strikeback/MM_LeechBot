...
class TorrentManager:
    ...
    async def initiate(cls):
        ...
        try:
            ...
            LOGGER.error(f"Error during torrent initialization: {e}")
            LOGGER.warning("Aria2/qBittorrent not available. Download features requiring these services will not work.")
            await cls.close_all()
    ...
    async def change_aria2_option(cls, key, value):
        if not cls.aria2:
            LOGGER.warning("Aria2 not initialized. Cannot change options.")
            return
        downloads = []
        results = await gather(cls.aria2.tellActive(), cls.aria2.tellWaiting(0, 1000))
        for res in results:
